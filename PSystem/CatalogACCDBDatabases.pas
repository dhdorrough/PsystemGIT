...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, MyDelimitedParser;
...
constructor TfrmCatalog.Create(aOwner: TComponent);
begin
  inherited;
...
//leRootFolder.Text     := FilerSettings.DebuggerDatabasesFolder;
  leOutputFileName.Text := FilerSettings.ReportsPath + 'Filer Databases Report.csv';
end;
...
procedure TfrmCatalog.btnBeginClick(Sender: TObject);
  ...
  var
    ...
    NrSegments, NrProcedures: integer;
    LatestProcDate, CurrentDateTime: TDateTime;
...
begin
  ...
  try
    ...
    lblStatus.Caption := Format('Processing complete. %d files processed. %d records processed',
