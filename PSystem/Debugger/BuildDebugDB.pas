...
{$endIf}
  OpsTables, Interp_Decl, ListingUtilitiesObject, BuildDbDb_Decl, Interp_Const;
...
//  procedure CreateDatabase(var FileName: string; DBVersion: TDBVersion);
  ...
  public
    ...
    Constructor Create2(aOwner: TComponent; TheDEBUGGERSettings: TDEBUGGERSettings);
    Destructor Destroy; override;
    procedure ScanCodeFileandUpdateDB;
  end;

var
  frmBuildDebugDB: TfrmBuildDebugDB;
...
procedure TfrmBuildDebugDB.UpdateProcInfo( var ProcedureInfo: TProcedureInfo;
                                                SegNameFull, ProcedureNameFull: string;
                                                aSegmentType: TType_of_Symbol;
                                                SegNumber: integer);
var
...
function TfrmBuildDebugDB.ProcessFile(VersionNr: TVersionNr; const InFileName: string): integer;
var
  Line: string;
//ProcNum,
...
*)
  ...
  var
    ...
    var
      Line: string;
    begin
      if frmFileParameters.FunctionType = ftBuildUglySource then
        Line := Source
      else
...
procedure TfrmBuildDebugDB.btnBrowseDBClick(Sender: TObject);
var
  FilePath: string;
begin
...
procedure TfrmBuildDebugDB.Open1Click(Sender: TObject);
var
  FilePath: string;
begin
  with frmFileParameters do
...
function TfrmBuildDebugDB.PrintResults(const OutputFileName: string): integer;
var
  sn, sn2, pn: integer;
  ProcedureInfo  : TProcedureInfo;
  SegmentInfo    : TSegmentInfo;
...
procedure TfrmBuildDebugDB.ParseandList1Click(Sender: TObject);
var
  ...
  try
    with frmFileParameters do
      begin
        ...
        InputListingFileName  := ListingsPath(VersionNr) + '\.' + TXT_EXT;
        DataBaseFileName      := fDEBUGGERSettings.DatabaseToUse;
        OutputFileName        := fDEBUGGERSettings.ReportsPath + 'BuildDB-parsed.' + CSV_EXT;
...
constructor TfrmBuildDebugDB.Create(aOwner: TComponent; TheDEBUGGERSettings: TDEBUGGERSettings);
begin
  inherited Create(aOwner);
  fDEBUGGERSettings := TheDEBUGGERSettings;
  FreeAndNil(frmFileParameters);
  frmFileParameters := TfrmFileParameters.Create(self);
  ...
  Memo1.Lines.Add('   by one of the pretty print programs (such as FORMAT.CODE) prior to compilation.');
  Memo1.Lines.Add('   Key words like PROGRAM, PROCEDURE, FUNCTION, CONST, VAR, TYPE, LABEL etc');
  Memo1.Lines.Add('   are expected to be the first word on a line.');
//Memo1.Lines.Add('2. It can also create the debug database from scratch.');
  Memo1.Lines.Add('3. It can scan a listing file and try to guess which compiler version created it.');
  Memo1.Lines.Add('4. It can try to clean up a compiler listing sent to CONSOLE: and write it to a new file.');
  Memo1.Lines.Add('');
end;

destructor TfrmBuildDebugDB.Destroy;
begin
  FreeAndNil(fSegmentInfoList);
  FreeAndNil(frmFileParameters);
...
procedure TfrmBuildDebugDB.BuildDatabasefromListing1Click(
  Sender: TObject);
begin
//if not Assigned(frmFileParameters) then
//  frmFileParameters := TfrmFileParameters.Create(self);
  with frmFileParameters do
    begin
      InputListingFileName  := ListingsPath(VersionNr) + '\.' + TXT_EXT;
      DataBaseFileName      := fDEBUGGERSettings.DatabaseToUse;
      OutputFileName        := fDEBUGGERSettings.ReportsPath + 'BuildDB.' + CSV_EXT;
      FunctionType          := ftBuildDbFromListing;
      if ShowModal = mrOK then
        begin
          if not FileExists(DatabaseFileName) then
            NewDatabase(DatabaseFileName);
          BuildDataBaseFromListing( InputListingFileName,
                                    DataBaseFileName,
                                    OutputFileName,
                                    ErasePCode);
        end;
...
procedure TfrmBuildDebugDB.MyUpdateStatus(Const Msg: string);
begin
  Memo1.Lines.Add(Msg);
...
procedure TfrmBuildDebugDB.NewDatabase(DatabaseFileName: string);
begin
...
procedure TfrmBuildDebugDB.miNewDatabaseClick(Sender: TObject);
begin
  with frmFileParameters do
    begin
...
procedure TfrmBuildDebugDB.Exit1Click(Sender: TObject);
begin
  Close;
end;

procedure TfrmBuildDebugDB.MyUpdateStatusFmt(const Msg: string;
  args: array of const);
begin
  MyUpdateStatus(Format(Msg, Args));
end;

function TfrmBuildDebugDB.GetWordAt(IPC: longword): word;
begin
  result := WordPtr(fBuffer + IPC)^;
end;

function TfrmBuildDebugDB.GetByteAt(IPC: longword): byte;
begin
  result := BytePtr(fBuffer + IPC)^;
end;

function TfrmBuildDebugDB.ProcedureIdentifier(const SegName: string; ProcedureInfo: TProcedureInfo): string;
//var
//Short, Long: string;
begin
...
procedure TfrmBuildDebugDB.MapSegmentsInCodeFile(Volume: TVolume; const pSysFileName : string;
                                                  OverWriteOptions: TOverWriteOptions;
                                                  VersionNr: TVersionNr);
type
  WordPtr = ^Word;
...
//                ProcedureInfo.ProcedureName := Format('PROC%2.2d', [ProcedureInfo.ProcedureNumber]);
                  ProcedureInfo.ProcedureName := Format('PROC%2.2d', [ProcNr]); //2/8/2024 
                ...
                AddLine(Format(
                         'Date decoded  : %s', [DateTimeToStr(Now())]));
    ...
    begin { ProcessUCSDSegment }
      ...
      if not Assigned(fPcodeDecoderII) then
...
function TfrmBuildDebugDB.GetBasedWord(Base, Offset: word): word;
begin
  result := WordPtr(fBuffer + Base + Offset)^;
end;

function TfrmBuildDebugDB.GetBasedByte(Base, Offset: word): byte;
begin
  result := BytePtr(fBuffer + Base + Offset)^;
end;


procedure TfrmBuildDebugDB.AddLine(const Line: string);
begin
  fDecodedLines.Add(TrimRight(Line));
end;

procedure TfrmBuildDebugDB.AddLineSeparator(OpCode: word);
begin
  with fOpsTable do
...
procedure TfrmBuildDebugDB.BuildDataBaseFromListing(const aListingFileName,
                                                          aDataBaseFileName,
                                                          aOutputFileName: string;
                                                    ErasePCode: boolean);
var
...
*)
  ...
  begin { UpdateVolumeInfoTable }
    with VolumeInfoTable do
      begin
        FilePath := fDEBUGGERSettings.LastBootedFileName;
...
begin { TfrmBuildDBDBMain.BuildDataBaseFromListing }
  ...
  Assert(not Assigned(fSegmentInfoList), 'System Error- fSgmentInfoList is not nil');  // this should not ever be true
  fSegmentInfoList := TSegmentInfoList.Create;
  frmUpdateConfirm := TfrmUpdateConfirm.Create(self, 'Procedures', '');
  try
    with frmFileParameters do
      begin
        VersionNr := fDebuggerSettings.VersionNr;
                          ...
                          with pCodesProcTable, ProcedureInfo do
                            begin
                              RecFound := Locate(IndexName_SEGNAME_PROC_NR_NAME_INDEX,
                                                 ...
                                                 [loCaseInsensitive]);
                              if not RecFound then // We may not have known the procedure name before. Try with just the number
                                  ...
                                  if sn2 >= 0 then
                                    begin
                                      ...
                                      if frmUpdateConfirm.OkToOverWriteWhat( What,
                                                                             OverWriteOptions,
                                                                             IIF(ErasePCode, '', fldDecodedPCode.AsString),
                                                                             ProcedureBody.Text,
                                                                             RecFound,
                                                                             ProcedureInfo) then
                                        begin
...
//                                fldSegmentNameFull.AsString   := xSegmentNAme;
                                  ...
                                  if ErasePCode then
                                    fldDecodedPCode.Clear;

                                  Temp := '';

                                  if not (VersionNr in  [vn_VersionI_5, vn_VersionI_5]) then
                                    MyUpdateStatusFmt('Variables in %s.%s may not be in correct order',
                                                      [xSegmentName, ProcedureName]);

                                  // REMINDER: This needs to write the variables in the order required by the VersionNr!
...
procedure TfrmBuildDebugDB.BuildUglySourceFromListing(const aListingFileName,
                                     aOutputFileName: string);
var
//sn, pn: integer;
  Processed: integer;
...
//ProcedureInfo  : TProcedureInfo;
  ...
  function ProcessTheFile(VersionNr: TVersionNr; const aListingFileName: string): integer;
  var
    Line: string;
    LineNumber: integer;
    SegNumber: integer;
  ...
  begin { ProcessTheFile}
    result := 0;
    try
      AssignFile(fInputFile, aListingFileName);
      Reset(fInputFile);
      AssignFile(fOutputFile, aOutputFileName);
      ReWrite(fOutputFile);
      while not Eof(fInputFile) do
        begin
          ReadLn(fInputFile, Line);
          if ProcessListingLine( VersionNr,
                                 Line,
                                 LineNumber,   // ignored
                                 SegNumber,    // ignored
                                 ProcNum,      // ignored
                                 ...
                                 Source,
                                 DataSeg,      // ignored,
                                 NestingLevel  // ignored
                                 ) then
            begin
              WriteLn(fOutputFile, Source);
              inc(result);
            end;
        end;
    finally
...
begin { BuildUglySourceFromListing }
  fSegmentInfoList := TSegmentInfoList.Create;
  try
    with frmFileParameters do
      begin
        Processed := ProcessTheFile(VersionNr, aListingFileName);
        Memo1.Lines.Add(Format('%d lines processed in file %s', [Processed, aListingFileName]));
        Application.ProcessMessages;
...
end;  { BuildUglySourceFromListing }


procedure TfrmBuildDebugDB.BuildUglySourceFromListing1Click(
  Sender: TObject);
begin
  with frmFileParameters do
    begin
...
procedure TfrmBuildDebugDB.VersionReporter( const line: string);
begin
  Memo1.Lines.Add(Line);
end;


procedure TfrmBuildDebugDB.ScanListingforVersionNumber1Click(
  Sender: TObject);
var
  ...
  try
    with frmFileParameters do
      begin
        ...
        InputListingFileName  := ListingsPath(VersionNr);
        OutputListingFileName := fDEBUGGERSettings.ReportsPath;
        if ShowModal = mrOK then
          begin
            with luo do
              begin
...
procedure TfrmBuildDebugDB.CleanupCompilerListingSentToCONSOLE1Click(
  Sender: TObject);
var
  ErrorCount: integer;
  luo: TListingUtilitiesObject;
...
(*
procedure TfrmBuildDebugDB.QDCleanUpOfProcedureNames1Click(
  Sender: TObject);
var
  aSegmentName: string;
  UpdateCnt : integer;
...
procedure TfrmBuildDebugDB.ScanCodeFileandUpdateDB;

var
  pSysFileName : string;
  dn: integer;
begin { ScanCodeFileandUpdateDB }
//if not Assigned(frmFileParameters) then
//  frmFileParameters := TFrmFileParameters.Create(self);
  with frmFileParameters do
    begin
      VersionNr        := fDebuggerSettings.VersionNr;
      DataBaseFileName := AccessDBFileFileName(VersionNr);
      OutputFileName   := Format('%sScanCodeAndUpdateDB.%s', [DefaultReportsPath, TXT_EXT]);
      ...
      if ShowModal = mrOK then
        begin
          ...
          WriteLn(fOutputFile, 'REMINDER: program may not handle WORD_MEMORY code properly.');
          WriteLn(fOutputFile);
...
end;  { ScanCodeFileandUpdateDB }

constructor TfrmBuildDebugDB.Create2(aOwner: TComponent;
  TheDEBUGGERSettings: TDEBUGGERSettings);
begin
  inherited Create(aOwner);
  fDEBUGGERSettings := TheDEBUGGERSettings;
  FreeAndNil(frmFileParameters);
