...
function  EditTextFile2(const EditorFilePathName, FileName: string): boolean;
...
function  HexBYTE(value:byte): string2;
...
function EditTextFile2(const EditorFilePathName, FileName: string): boolean;
begin
  result := ExecAndWait(EditorFilePathName, FileName, true);
end;
...
begin
  if Length(FileName) <= HashLen then
    ...
    begin
      ...
      if Length(Ext) > EXTMAX then
        Ext := Copy(Ext, 1, EXTMAX);
        
      Ext     := FixExt(Ext);       // get rid of the '.'

      ExtLen  := Length(Ext);
      if (ExtLen > 0) and (ExtLen < EXTMAX) then
        Ext := Padr(Ext, EXTMAX);
      ...
      if ExtLen > 0 then
        result := Left(Base, PfxLen) + Right(Base, PfxLen) + Ext
      else
...
//   Useage            : if BrowseForFile('Specify file', FilePath, 'ACCDB'
//   Function Purpose  :
